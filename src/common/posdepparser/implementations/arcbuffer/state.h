...
public:
   ...
   inline bool canAssignArc() const
   {
	   if( m_nStackSize > 1 && (m_nBufferSize == action::max_buffer_size ||  m_nQNextWord == m_lCache->size()))
	   {
		   return true;
	   }
	   else
   ...
   inline bool canShiftNone() const
   {
	   if( m_nBufferSize > 0 && (m_nBufferSize == action::max_buffer_size ||  m_nQNextWord == m_lCache->size()))
	   {
		   return true;
	   }
	   else
   ...
   inline bool canShiftPOS() const
   {
	   if( (m_nBufferSize < action::max_buffer_size &&  m_nQNextWord < m_lCache->size()) || action::max_buffer_size == 0)
	   {
		   return true;
	   }
	   else
   ...
   inline bool canPopRoot() const
   {
	   if( m_nHeadSetSize == m_nNextWord -1 && m_nNextWord == m_nQNextWord && m_nNextWord == m_lCache->size())
	   {
		   return true;
	   }
	   else
...
public:
   ...
   void ArcLeft() { 
#endif
      assert( m_nStackSize > 1 && (  m_nBufferSize == action::max_buffer_size ||  m_nQNextWord == m_lCache->size()) ) ;
      static int top0, top1;
      top0 = m_Stack[m_nStackSize-1];
      top1 = m_Stack[m_nStackSize-2];
      assert(m_lHeads[top1] == DEPENDENCY_LINK_NO_HEAD);
      m_nStackSize--;
      ...
      //if(m_nBufferSize > 0)
      //{
 		// int b0word_index = m_Buffer[0];
 		//  m_nStackSize++;
 	    //  m_Stack[m_nStackSize-1] = b0word_index;
 	    //  for(int idx = 0; idx < m_nBufferSize -1; idx++)
 	    //  {
 	    //	  m_Stack[idx] = m_Stack[idx+1];
 	    //  }
 	    //  m_nBufferSize--;
 	    // m_nNextWord++;
     // }
   ...
   void ArcRight() { 
      ...
      top0 = m_Stack[m_nStackSize-1];
      top1 = m_Stack[m_nStackSize-2];
      assert(m_lHeads[top0] == DEPENDENCY_LINK_NO_HEAD);
      m_nStackSize--;
   ...
   void Shift(unsigned long lab) {
	  if(action::max_buffer_size > 0 )
	  {
		  if(lab > 0)
		  {
			  assert(m_nBufferSize  < action::max_buffer_size || m_nQNextWord < m_lCache->size());
			  m_nBufferSize++;
		  ...
		  {
			  assert(lab == 0 && (m_nBufferSize  == action::max_buffer_size || m_nQNextWord == m_lCache->size()));
			  m_nStackSize++;
	  ...
	  {
		  assert( m_nQNextWord == m_nNextWord && m_nQNextWord < m_lCache->size());
		  m_nStackSize++;
...
public:
      ...
      SCORE_TYPE hammingloss(const CDependencyParse &tree, const unsigned long &ac)  const
      {
    	  SCORE_TYPE loss = 0;
#ifdef LABELED
      switch (action::getUnlabeledAction(ac)) {
      ...
      case action::SHIFT:
    	  if(ac != action::SHIFT)
    	  {
			  if(CTag(tree[m_nQNextWord].tag).code() == action::getLabel(ac))
			  {
				  return 0;
			  }
			  else
    	  ...
    	  else
    	  {
    		  ...
    		  {
    			  if (tree[m_Stack[i]].head == m_Stack[m_nStackSize-1])
    			  {
    				  loss = loss +1;
    			  }
    		  }
      ...
      case action::ARC_LEFT:
       	  ...
       	  for( int i = 0; i < m_nStackSize - 2; i++)
		  {
			  if (tree[m_Stack[i]].head == m_Stack[m_nStackSize-2] || tree[m_Stack[m_nStackSize-2]].head == m_Stack[i])
			  {
				  loss = loss +1;
			  }
		  }
       	  if(tree[m_Stack[m_nStackSize-2]].head != m_Stack[m_nStackSize-1])
       	  {
       		loss = loss +1;
       	  }
       	  for(int i = m_nNextWord; i <  m_lCache->size(); i++)
       	  {
			  if (tree[i].head == m_Stack[m_nStackSize-2] || tree[m_Stack[m_nStackSize-2]].head == i)
			  {
				  loss = loss +1;
			  }
       	  }
#ifdef LABELED
         if(CDependencyLabel(tree[m_Stack[m_nStackSize-2]].label).code() != action::getLabel(ac))
         {
        	 loss = loss +1;
         }
#endif
      ...
      case action::ARC_RIGHT:
       	  for( int i = 0; i < m_nStackSize - 2; i++)
		  {
			  if (tree[m_Stack[i]].head == m_Stack[m_nStackSize-1] || tree[m_Stack[m_nStackSize-1]].head == m_Stack[i])
			  {
				  loss = loss +1;
			  }
		  }
       	  if(tree[m_Stack[m_nStackSize-1]].head != m_Stack[m_nStackSize-2])
       	  {
       		loss = loss +1;
       	  }
       	  for(int i = m_nNextWord; i <  m_lCache->size(); i++)
       	  {
			  if (tree[i].head == m_Stack[m_nStackSize-1] || tree[m_Stack[m_nStackSize-1]].head == i)
			  {
				  loss = loss +1;
			  }
       	  }
#ifdef LABELED
         if(CDependencyLabel(tree[m_Stack[m_nStackSize-1]].label).code() != action::getLabel(ac))
         {
        	 loss = loss +1;
         }
#endif
         return loss;
      case action::POP_ROOT:
      	  if(tree[m_Stack[m_nStackSize-1]].head != -1)
       	  {
      		  return 1;
       	  }
      	  else
...
#endif
      assert(tree.size() == m_lCacheLabel.size() && tree.size() == m_lCache->size());
      static int top0, top1;
