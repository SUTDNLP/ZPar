...
public:
   ...
   CStateNode() : id(-1), type(), temp(0), constituent(), left_child(0), right_child(0), lexical_head(0), lexical_start(0), lexical_end(0), level_left(0), level_right(0) {}
   virtual ~CStateNode() {}
public:
   ...
   void operator = (const CStateNode &nd) {
      ...
      lexical_start = nd.lexical_start;
      lexical_end = nd.lexical_end;
      level_left = nd.level_left;
      level_right = nd.level_right;
   }
public:
...
protected:
   ...
   void shift(CStateItem *retval, const unsigned long &constituent = CConstituent::NONE) const {
      ...
      retval->node.level_left = 0;
      retval->node.level_right = 0;
   ...
   void reduce(CStateItem *retval, const unsigned long &constituent, const bool &single_child, const bool &head_left, const bool &temporary) const {
	 ...
	 retval->node.level_left = l->level_left+1;
	 retval->node.level_right = l->level_right+1;
#ifdef TRAIN_LOSS
         retval->bTrain = this->bTrain;
	 ...
	 if(temporary == false){
           retval->node.level_left = l->level_left+1;
	   retval->node.level_right = r->level_right+1;
	 }
	 else{
	   retval->node.level_left = l->level_left;
           retval->node.level_right = r->level_right;
  	 }
#ifdef TRAIN_LOSS
         retval->bTrain = this->bTrain;
