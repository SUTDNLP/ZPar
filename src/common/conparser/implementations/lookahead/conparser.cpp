...
//   s0cs1c_distaction = encodeAction(action, m_Context.s0cs1c_dist);
   ...
   static CConstituent constituent_tmp;
  // std::cerr<<m_extra_all.size()<<std::endl;
//	std::cerr<<"========================================\n";
	if(item->stacksize() != 0){
//		std::cerr<<"S0 "<<(item->node).lexical_start<<" "<<(item->node).lexical_end<<" "<<(item->node).lexical_head<<std::endl;
//		std::cerr<<"level "<<item->node.level_left<<" "<<item->node.level_right<<"\n";
		const std::vector<int> &labels_left = m_extra_left[(item->node).lexical_start];
		const int& label_left = (item->node).level_left < labels_left.size() ? labels_left[(item->node).level_left] : 1;
		if(label_left != 1){
		constituent_tmp.load(label_left);
//		std::cerr<<constituent_tmp<<"\n";
		extra_fire += 1;
		cast_weights->m_mapS0lcd.getOrUpdateScore(retval, constituent_tmp, action.code(), m_nScoreIndex, amount, round);
		}

		const std::vector<int> &labels_right = m_extra_right[(item->node).lexical_end];
		const int& label_right = (item->node).level_right < labels_right.size() ? labels_right[(item->node).level_right] : 1;
		if(label_right != 1){
		constituent_tmp.load(label_right);
//              std::cerr<<constituent_tmp<<"\n";
		extra_fire += 1;
		cast_weights->m_mapS0rcd.getOrUpdateScore(retval, constituent_tmp, action.code(), m_nScoreIndex, amount, round);
		}
//		std::cerr<<"done"<<std::endl;
/*		const unsigned long &S0la = m_extra_all[(item->node).lexical_start];
		const unsigned long &S0ra = m_extra_all[(item->node).lexical_end];
		cast_weights->m_mapS0la.getOrUpdateScore(retval, S0la, action.code(), m_nScoreIndex, amount, round);
		cast_weights->m_mapS0ra.getOrUpdateScore(retval, S0ra, action.code(), m_nScoreIndex, amount, round);
*/
	}
	if(m_Context.s1 !=0){
//		std::cerr<<"S1 "<<(m_Context.s1)->lexical_start<<" "<<(m_Context.s1)->lexical_end<<" "<<(m_Context.s1)->lexical_head<<std::endl;
//		std::cerr<<"level "<<(m_Context.s1)->level_left<<" "<<(m_Context.s1)->level_right<<"\n";
		const std::vector<int> &labels_left = m_extra_left[(m_Context.s1)->lexical_start];
                const int& label_left = (m_Context.s1)->level_left < labels_left.size() ? labels_left[(m_Context.s1)->level_left] : 1;
                if(label_left != 1){
		constituent_tmp.load(label_left);
//		std::cerr<<constituent_tmp<<"\n";
                extra_fire += 1;
                cast_weights->m_mapS1lcd.getOrUpdateScore(retval, constituent_tmp, action.code(), m_nScoreIndex, amount, round);
//		std::cerr<<"done"<<std::endl;
		}
		const std::vector<int> &labels_right = m_extra_right[(m_Context.s1)->lexical_end];
                const int& label_right = (m_Context.s1)->level_right < labels_right.size() ? labels_right[(m_Context.s1)->level_right] : 1;
                if(label_right != 1){
                constituent_tmp.load(label_right);
//              std::cerr<<constituent_tmp<<"\n";
                extra_fire += 1;
                cast_weights->m_mapS1rcd.getOrUpdateScore(retval, constituent_tmp, action.code(), m_nScoreIndex, amount, round);
                }
/*		const unsigned long &S1la = m_extra_all[(m_Context.s1)->lexical_start];
		const unsigned long &S1ra = m_extra_all[(m_Context.s1)->lexical_end];
		cast_weights->m_mapS1la.getOrUpdateScore(retval, S1la, action.code(), m_nScoreIndex, amount, round);
		cast_weights->m_mapS1ra.getOrUpdateScore(retval, S1ra, action.code(), m_nScoreIndex, amount, round);
*/	}
	if(m_Context.s2 !=0){
//		std::cerr<<"S2 "<<(m_Context.s2)->lexical_start<<" "<<(m_Context.s2)->lexical_end<<" "<<(m_Context.s2)->lexical_head<<std::endl;
//		std::cerr<<"level "<<(m_Context.s2)->level_left<<" "<<(m_Context.s2)->level_right<<"\n";
//		 std::cerr<<"done"<<std::endl;
	}
	if(m_Context.s3 != 0){
//		std::cerr<<"S3 "<<(m_Context.s3)->lexical_start<<" "<<(m_Context.s3)->lexical_end<<" "<<(m_Context.s3)->lexical_head<<std::endl;
//		std::cerr<<"level "<<(m_Context.s3)->level_left<<" "<<(m_Context.s3)->level_right<<"\n";
	}
	if(m_Context.n0!=-1){
//		std::cerr<<"N0 "<<m_Context.n0<<std::endl;
		const std::vector<int> &labels_left = m_extra_left[m_Context.n0];
                const int& label_left = 0 < labels_left.size() ? labels_left[0] : 1;
                if(label_left != 1){
		constituent_tmp.load(label_left);
//		std::cerr<<constituent_tmp<<"\n";
                extra_fire += 1;
                cast_weights->m_mapN0lcd.getOrUpdateScore(retval, constituent_tmp, action.code(), m_nScoreIndex, amount, round);
		}
		const std::vector<int> &labels_right = m_extra_right[m_Context.n0];
                const int& label_right = 0 < labels_right.size() ? labels_right[0] : 1;
                if(label_right != 1){
                constituent_tmp.load(label_right);
//              std::cerr<<constituent_tmp<<"\n";
                extra_fire += 1;
                cast_weights->m_mapN0rcd.getOrUpdateScore(retval, constituent_tmp, action.code(), m_nScoreIndex, amount, round);
                }
		extra_fire += 1;
		cast_weights->m_mapN0have.getOrUpdateScore(retval, true, action.code(), m_nScoreIndex, amount, round);
//		std::cerr<<"done"<<std::endl;
	/*	const std::vector<std::string> &labels_to_end = m_extra[m_Context.n0];
		for(int idx = 0; idx < labels_to_end.size(); idx ++){
			const std::string &label = labels_to_end[idx];
			std::map<std::string,int>::iterator iter = extra_map.find(label);
			if(iter != extra_map.end()){
				(cast_weights->m_mapNcd[iter->second])->getOrUpdateScore(retval, true, action.code(), m_nScoreIndex, amount, round);
			}
		}*/
//		const unsigned long &N0a = m_extra_all[m_Context.n0];
//		 cast_weights->m_mapN0a.getOrUpdateScore(retval, N0a, action.code(), m_nScoreIndex, amount, round);
	}
	if(m_Context.n1!=-1){
//		std::cerr<<"N1 "<<(m_Context.n1)<<std::endl;
		const std::vector<int> &labels_left = m_extra_left[m_Context.n1];
                const int& label_left = 0 < labels_left.size() ? labels_left[0] : 1;
                if(label_left != 1){
		constituent_tmp.load(label_left);
//		std::cerr<<constituent_tmp<<"\n";
                extra_fire += 1;
                cast_weights->m_mapN1lcd.getOrUpdateScore(retval, constituent_tmp, action.code(), m_nScoreIndex, amount, round);
		}
		const std::vector<int> &labels_right = m_extra_right[m_Context.n1];
                const int& label_right = 0 < labels_right.size() ? labels_right[0] : 1;
                if(label_right != 1){
                constituent_tmp.load(label_right);
//              std::cerr<<constituent_tmp<<"\n";
                extra_fire += 1;
                cast_weights->m_mapN1rcd.getOrUpdateScore(retval, constituent_tmp, action.code(), m_nScoreIndex, amount, round);
                }
		extra_fire += 1;
		cast_weights->m_mapN1have.getOrUpdateScore(retval, true, action.code(), m_nScoreIndex, amount, round);
//		std::cerr<<"done"<<std::endl;
	}
	if(m_Context.n2!=-1){
//		std::cerr<<"N2 "<<(m_Context.n2)<<std::endl;
		extra_fire += 1;
		cast_weights->m_mapN2have.getOrUpdateScore(retval, true, action.code(), m_nScoreIndex, amount, round);
//		std::cerr<<"done"<<std::endl;
	}
	if(m_Context.n3!=-1){
//		std::cerr<<"N3 "<<(m_Context.n3)<<std::endl;
		extra_fire += 1;
		cast_weights->m_mapN3have.getOrUpdateScore(retval, true, action.code(), m_nScoreIndex, amount, round);
//		 std::cerr<<"done"<<std::endl;
	}
	extra_parse_time += double(clock() - feature_time)/CLOCKS_PER_SEC;
// CWeight* cast_weights = (amount&&(round!=-1)) ? m_delta : static_cast<CWeight*>(m_weights);
   ...
   if (!m_Context.s0c.empty()) {fire+=1;cast_weights->m_mapS0c.getOrUpdateScore(retval, m_Context.s0c, action.code(), m_nScoreIndex, amount, round);}
   ...
   if (m_Context.s1!=0) {
      ...
      if (!m_Context.s1c.empty()) {fire += 1;cast_weights->m_mapS1c.getOrUpdateScore(retval, m_Context.s1c, action.code(), m_nScoreIndex, amount, round);}
...
/* BRACKET PUNC */
   ...
   parse_time += double(clock() - feature_time)/CLOCKS_PER_SEC;
...
//         updateScoresByLoss(pBestGen, correctState) ;
   ...
   if(nBest == AGENDA_SIZE){
	 int i = 0;
	 for(CStateItem *p = lattice_index[index]+1; p != lattice_index[index+1]; p ++){
...
void CConParser::setExtra( const std::vector<std::vector<std::string> > &extra){
	m_extra_left.clear();
	m_extra_left.resize(extra.size());
	m_extra_right.clear();
        m_extra_right.resize(extra.size());
	m_extra_head.clear();
	m_extra_head.resize(extra.size());
	
	for(int i = 0; i < extra.size(); i ++){
		m_extra_left[i].clear();
		m_extra_right[i].clear();
		m_extra_head[i].clear();
		
		for(int j = 0; j < extra[i].size(); j ++){
			std::map<std::string,int>::iterator iter;	
		
 			const std::string& label = extra[i][j];
			iter = extra_start_map.find(label);
			if(iter != extra_start_map.end()){
				m_extra_left[i].push_back((iter->second+2)%CConstituent::COUNT);
			}
			iter = extra_end_map.find(label);
                        if(iter != extra_end_map.end()){
                                m_extra_right[i].push_back((iter->second+2)%CConstituent::COUNT);
			}
			iter = extra_head_map.find(label);
                        if(iter != extra_head_map.end()){
                                m_extra_head[i].push_back((iter->second+2)%CConstituent::COUNT);
			}
		}
	}
}
