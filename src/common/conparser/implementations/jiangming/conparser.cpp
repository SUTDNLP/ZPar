...
//		   std::cerr<<"ok0"<<std::endl;
	  ...
	  for(const CStateItem* p = lattice_index[index]; p!= lattice_index[index+1]; p ++, i++){
		...
		const CStateItem *t = p;
		while(t->statePtr){
			acts.push_back(t->action);
			t = t->statePtr;
		}
		...
		for(int j = 0; j < acts.size(); j ++){
			CStateItem* from = &items[j];
			CStateItem* to = &items[j+1];
			from->Move_arc(to,acts[j]);
		}
		if(o_conll != 0)
		items[acts.size()].GenerateStanford(sentence, o_conll+i);
		if (scores)
		scores[i] = p->score;
	  }
   }
   ...
   std::vector<CAction> acts;
   while(pBestGen->statePtr){
   	acts.push_back(pBestGen->action);
	pBestGen = pBestGen->statePtr;
   }
   std::reverse(acts.begin(),acts.end());
   std::vector<CStateItem> items(acts.size()+1);
   items[0].clear();
