...
void auto_train(const std::string &sOutputFile, const std::string &sFeatureFile, const std::string &sExtraFile, const std::string &sBinaryRulePath, const std::string &sUnaryRulePath, const std::string &sConInputPath) {
...
int main(int argc, char* argv[]) {
   ...
   try {
      ...
      if (options.args.size() != 5) {
         std::cout << "\nUsage: " << argv[0] << " training_data model num_iterations extra_softfeature_file" << std::endl ;
      ...
      for (int i=0; i<training_rounds; ++i) {
         auto_train(options.args[1], options.args[2], options.args[4], sBinaryRulePath, sUnaryRulePath, sConInputPath); // set update tag dict false now
